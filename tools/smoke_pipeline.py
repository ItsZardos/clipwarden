...
_ENV_KEYS = ("CLIPWARDEN_APPDATA", "CLIPWARDEN_DEMO_MODE")
...
def _restore_env(snapshot: dict[str, str | None]) -> None:
    """Undo :func:`_reset_appdata` so a REPL session or test runner
    invoking ``main()`` doesn't leak ``CLIPWARDEN_APPDATA`` /
    ``CLIPWARDEN_DEMO_MODE`` into subsequent imports of
